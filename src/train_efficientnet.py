...
==========================
EfficientNet-B0 Training Pipeline for HAM10000
-----------------------------------------------
- Loads train / val splits from  data_splits/
- Uses the shared HAM10000Dataset + Albumentations transforms
- Fine-tunes an EfficientNet-B0 (ImageNet pre-trained) on 7 classes
- Applies class-balanced CrossEntropyLoss
- Uses AdamW optimizer + CosineAnnealingLR scheduler
- Tracks top-1 validation accuracy and saves best checkpoint to:
    checkpoints/efficientnet_best.pth

Usage:
    python src/train_efficientnet.py [--epochs 25] [--batch_size 32] [--lr 1e-4]
"""
...
# Add project root to path so imports resolve when run from any CWD
...
def parse_args():
    p = argparse.ArgumentParser(description="Train EfficientNet-B0 on HAM10000")
    p.add_argument("--train_csv",   default="data_splits/train_split.csv")
    p.add_argument("--val_csv",     default="data_splits/val_split.csv")
    p.add_argument("--checkpoint",  default="checkpoints/efficientnet_best.pth")
...
def main():
    ...
    print(f"[EfficientNet] Train samples : {len(train_ds)}")
    print(f"[EfficientNet] Val   samples : {len(val_ds)}")
    ...
    print(f"\n[EfficientNet] Training complete. Best val acc: {best_val_acc:.4f}")
    return history
